...
void newIndiv (int *idGlobal, int *COVIDentry, int *COVIDentryAb, int currDay, sIndiv **pHeadCopy, sIndiv **pTailCopy, gsl_rng **r, int nEvents, int (*pLocArray2)[ROWPRIS][COLCTR], int prison, int category, int seedPop, int seedZone, int (*pMinCellArray)[27][2][6][13], int (*pMedCellArray)[11][2][4][19], int (*pMaxCellArray)[18][4][5][20], int (*pIsolateMinZone)[27], int (*pIsolateMedZone)[11], int (*pIsolateMaxZone)[18], int (*pIsolateMinArea)[27][2], int (*pIsolateMedArea)[11][2], int (*pIsolateMaxArea)[18][4], int (*pIsolateMinUnit)[27][2][6], int (*pIsolateMedUnit)[11][2][4], int (*pIsolateMaxUnit)[18][4][5], int (*pIsolateMinCell)[27][2][6][13], int (*pIsolateMedCell)[11][2][4][19], int (*pIsolateMaxCell)[18][4][5][20], int *povercap){
    ...
    //SET GENDER
    obj->vaccineProtection=0.0;
    obj->vaccineTransmissionReduction=0.0;
    obj->vaxDose1=0;
    obj->vaxDose2=0;
    obj->vaxDose1Day=0;
    obj->vaxDose2Day=0;
    ...
    *pTailCopy=obj;
    ...
    //set vax
    if(obj->indivType==0){
        communityVaxRate=0.80;
        
        //prob of at least one dose
            probsRisk[0]=1-communityVaxRate;//.20//1-0.164; //probability of not being vaxx
            probsRisk[1]=communityVaxRate;//0.20//0.164; //probability of vaxx
            binVax=draw_multinom(&rCopy, 2, probsRisk);
        
        if(binVax==1){
            //double dose vaxx
            communityVaxRate=1.0; //to get 50% of population double dosed
            
                probsRisk[0]=1-communityVaxRate;//.20//1-0.164; //probability of not being double vaxx
                probsRisk[1]=communityVaxRate;//0.20//0.164; //probability of double vaxx
                binVax=draw_multinom(&rCopy, 2, probsRisk);
            
            if(binVax==1){ //double vaxxed
                obj->vaccineProtection=0.79;
                obj->vaccineTransmissionReduction=0.65;
                obj->vaxDose1=1;
                obj->vaxDose2=1;
                obj->vaxDose1Day=currDay;
                obj->vaxDose2Day=currDay;
            }else{ //single vaxxed
                obj->vaccineProtection=0.30;
                obj->vaccineTransmissionReduction=0.46;
                obj->vaxDose1=1;
                obj->vaxDose2=0;
                obj->vaxDose1Day=currDay;
                obj->vaxDose2Day=currDay;
        ...
        if(binVax==1){
            //double dose vaxx
            communityVaxRate=0.625; //to get 50% of population double dosed
...
void vaxInmates (sIndiv **pHeadCopy, sIndiv **pTailCopy, int currDay, int *inmateDose1cap, int *inmateVaxD1, int *inmateVaxD2){
    sIndiv *current;
    current=*pHeadCopy;
    int deltaTime=0;
    ...
    while(current!=NULL){
        ...
        if(current->indivType==0){
            if(current->vaxDose1==1&&current->vaxDose2==0){ //if vaxx 1 done, check if delta time is enough
                deltaTime=currDay-(current->vaxDose1Day);
                if(deltaTime>=21){
                    if((*inmateDose1cap)>0){    //check if dose2cap is not yet 0
                        current->vaxDose2=1;
                        current->vaxDose2Day=currDay;
                        current->vaccineProtection=0.79;
                        current->vaccineTransmissionReduction=0.65;
                        (*inmateDose1cap)--;
                        (*inmateVaxD2)++;
                    }
                }
            }else if(current->vaxDose1==0){ //if vaxx 1 not yet done
                if((*inmateDose1cap)>0){    //check if dose1cap is not yet 0
                    current->vaxDose1=1;
                    current->vaccineProtection=0.30;
                    current->vaccineTransmissionReduction=0.46;
                    (*inmateDose1cap)--;
                    (*inmateVaxD1)++;
                }
            }
...
void vaxPStaff (sIndiv **pHeadCopy, sIndiv **pTailCopy, int currDay, int *psDose1cap, int *psVaxD1, int *psVaxD2){
    sIndiv *current;
    current=*pHeadCopy;
    int deltaTime=0;
    int testDate=current->testDate;
    ...
    while(current!=NULL){
        ...
        if(current->indivType==1){
            if(current->vaxDose1==1&&current->vaxDose2==0){ //if vaxx 1 done, check if delta time is enough
                deltaTime=currDay-(current->vaxDose1Day);
                if(deltaTime>=21){
                    if((*psDose1cap)>0){    //check if dose2cap is not yet 0
                        current->vaxDose2=1;
                        current->vaxDose2Day=currDay;
                        current->vaccineProtection=0.79;
                        current->vaccineTransmissionReduction=0.65;
                ...
                if((*psDose1cap)>0){    //check if dose1cap is not yet 0
                    current->vaxDose1=1;
                    current->vaccineProtection=0.30;
                    current->vaccineTransmissionReduction=0.46;
                    (*psDose1cap)--;
...
void vaxHStaff (sIndiv **pHeadCopy, sIndiv **pTailCopy, int currDay, int *hsDose1cap, int *hsVaxD1, int *hsVaxD2){
    sIndiv *current;
    current=*pHeadCopy;
    int deltaTime=0;
    int testDate=current->testDate;
    ...
    while(current!=NULL){
        ...
        if(current->indivType==2){
            if(current->vaxDose1==1&&current->vaxDose2==0){ //if vaxx 1 done, check if delta time is enough
                deltaTime=currDay-(current->vaxDose1Day);
                if(deltaTime>=21){
                    if((*hsDose1cap)>0){    //check if dose2cap is not yet 0
                        current->vaxDose2=1;
                        current->vaxDose2Day=currDay;
                        current->vaccineProtection=0.79;
                        current->vaccineTransmissionReduction=0.65;
                ...
                if((*hsDose1cap)>0){    //check if dose1cap is not yet 0
                    current->vaxDose1=1;
                    current->vaccineProtection=0.30;
                    current->vaccineTransmissionReduction=0.46;
                    (*hsDose1cap)--;
...
void countVax (sIndiv **pHeadCopy, sIndiv **pTailCopy, int *InmateV1, int *InmateV2, int *PSV1, int *PSV2, int *HSV1, int *HSV2){
    ...
    while(current!=NULL){
        ...
        if(current->vaxDose1==1&&current->vaxDose2==0){ //half vaxx
            if(current->indivType==0){
                (*InmateV1)++;
        ...
        }else if(current->vaxDose1==1&&current->vaxDose2==1){ //full vaxx
            if(current->indivType==0){
                (*InmateV2)++;
            }else if(current->indivType==1){
                (*PSV2)++;
...
int infectRedZone (gsl_rng **r, sIndiv **pTargetCopy, sIndiv **pHeadCopy, sIndiv **pTailCopy, int (*pLocArray2)[ROWPRIS][COLCTR], int (*zoneMinCIArray)[30][7], int (*zoneMedCIArray)[30][7], int (*zoneMaxCIArray)[30][7], int *newInfectedInmates, int *newInfectedPS, int *newInfectedHS, int *newInfectedEV, int *newInfectedFV, int *newInfectedInmatesMin, int *newInfectedPSMin, int *newInfectedHSMin, int *newInfectedEVMin, int *newInfectedFVMin, int *newInfectedInmatesMed, int *newInfectedPSMed, int *newInfectedHSMed, int *newInfectedEVMed, int *newInfectedFVMed, int *newInfectedInmatesMax, int *newInfectedPSMax, int *newInfectedHSMax, int *newInfectedEVMax, int *newInfectedFVMax,  int *totalAge0, int *totalAge1, int *totalAge2, int *totalAge3, int *totalAge4, int *totalAge5, int *totalAge6, int *exp19, int *exp20to44, int *exp45to54, int *exp55to64, int *exp65to74, int *exp75to84, int *exp85, int currDay){
    ...
    while(tempCurrent!=NULL){
        ...
        protection=tempCurrent->vaccineProtection;
                                ...
                                if(tempCurrent->locArea==target->locArea&&tempCurrent->locUnit==target->locUnit&&tempCurrent->locCell==target->locCell){
                                    ...
                                    if(tempCurrent->COVID==0&&tempCurrent->COVIDAb==1){ //immunity
                                        ...
                                        if(target->severity==4||target->severity==5){ //mod or severe
                                            ...
                                            probInf[1]=probInf[1]-(probInf[1]*0.85); //assuming face mask at all times
                                            probInf[1]=probInf[1]-(probInf[1]*protection); //vaccine
...
double probInfectIndiv(sIndiv **pTarget, int currDay){ //Infected can infect everyone so 1 if symptomatic
    sIndiv *current;
    current=*pTarget;
    double rN=0.0;
    double transmissionReduction;
    int infTime;
    infTime=current->timeOfInfection;

    transmissionReduction=current->vaccineTransmissionReduction;
        ...
        if(current->hospitalised==0){ //&&current->isolated==0
            if(current->severity>=2&&current->severity<=5){ //asymptomatic to severe
                if(current->indivType==0){ //individual
                    ...
                    rN=1.0-0.0504;//1.0-0.8; //assuming they are wearing PPE
                    rN=rN-transmissionReduction;
                }else if(current->indivType==3){ //essential visitor
...
double probProgress(gsl_rng **r, sIndiv **pTarget, int currDay){ //Probability to progress metavir scale
    ...
    if(current->COVID==1){
        ...
        if(current->severity==0){ //exposed -> pre-clinical
            ...
            reduction=0.0;
            if(current->vaxDose1==1){
                if(current->vaxDose2==1){ //fullvax
                    reduction=0.87;
                }else{ //halfvax
                    reduction=0.70;
                }
    ...
    if(newLoc!=current->location){
        if(newLoc==0){
            ...
            if(newLoc==1){ //objLoc==1||objLoc==6||objLoc==11
                ...
                /*
                //set units
                //current->locUnit=generateAge(1, 13);
                probsMinArea[0]=0.5;
                probsMinArea[1]=0.5;
                current->locArea=draw_multinom(&rCopy, 2, probsMinArea);
                //obj->locArea=generateAge(1, 2);
                ...
                probsMinUnit[4]=0.16666666666;
                probsMinUnit[5]=0.16666666666;
                current->locUnit=draw_multinom(&rCopy, 6, probsMinUnit);
                //obj->locUnit=generateAge(1, 6);
                ...
                cellCap=0;
                while(cellFlag==0){
                    current->locCell=draw_multinom(&rCopy, 13, probsMinCell);
                    
                    cellCap=(*pMinCellArray)[current->zone][current->locArea][current->locUnit][current->locCell];
                    if(cellCap<2){
                        cellFlag=1;
                    }
                }
                ...
                //current->locCell=draw_multinom(&rCopy, 13, probsMinCell);
                //current->locArea=generateAge(1, 2);
                //current->locUnit=generateAge(1, 6);
                //current->locCell=generateAge(1, 13);
                */
                ...
                /*
                //set units
                //current->locUnit=generateAge(1, 4);
                probsMedArea[0]=0.5;
                probsMedArea[1]=0.5;
                current->locArea=draw_multinom(&rCopy, 2, probsMedArea);
                //obj->locArea=generateAge(1, 2);
                ...
                probsMedUnit[2]=0.16666666666;
                probsMedUnit[3]=0.16666666666;
                current->locUnit=draw_multinom(&rCopy, 4, probsMedUnit);
                //obj->locUnit=generateAge(1, 6);
                ...
                while(cellFlag==0){
                    ...
                    cellCap=(*pMedCellArray)[current->zone][current->locArea][current->locUnit][current->locCell];
                    if(cellCap<2){
                        cellFlag=1;
                    }
                }
                ...
                //current->locArea=generateAge(1, 2);
                //current->locUnit=generateAge(1, 4);
                //current->locCell=generateAge(1, 19);
                ...
                probsMaxArea[2]=0.25;
                probsMaxArea[3]=0.25;
                current->locArea=draw_multinom(&rCopy, 4, probsMaxArea);
                //obj->locArea=generateAge(1, 2);
                ...
                probsMaxUnit[3]=0.2;
                probsMaxUnit[4]=0.2;
                current->locUnit=draw_multinom(&rCopy, 5, probsMaxUnit);
                //obj->locUnit=generateAge(1, 6);
                ...
                cellCap=0;
                while(cellFlag==0){
                    current->locCell=draw_multinom(&rCopy, 20, probsMaxCell);
                    
                    cellCap=(*pMaxCellArray)[current->zone][current->locArea][current->locUnit][current->locCell];
                    if(cellCap<2){
                        cellFlag=1;
                    }
                }
                ...
                //current->locCell=draw_multinom(&rCopy, 20, probsMaxCell);
                //current->locArea=generateAge(1, 4);
                //current->locUnit=generateAge(1, 5);
                //current->locCell=generateAge(1, 20);
                            ...
                            if(target->indivType==0){
                                if(tempCurrent->locArea==target->locArea){
                                    ...
                                    if(tempCurrent->COVID==0&&tempCurrent->COVIDAb==1){ //immunity
                                        ...
                                        if(target->severity==4||target->severity==5){ //mod or severe
                                            probInf[1]=0.01*2;//0.001;//0.05;//target = probability of getting infected with COVID-19
                                            probInf[1]=probInf[1]-(probInf[1]*0.67); //assuming face mask at all times
                                            probInf[1]=probInf[1]-(probInf[1]*protection); //vaccine
                                        }else{ //mild and asymptomatic
                                            probInf[1]=(gsl_ran_flat(*r, 0.02, 0.05))*2;//0.05;//0.05;//probability of getting infected with COVID-19
                                            probInf[1]=probInf[1]-(probInf[1]*0.67); //assuming face mask at all times
