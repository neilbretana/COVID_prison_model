...
double probMoveP1(gsl_rng **r, sIndiv **pTarget, int currDay); //15
double probMoveP2(gsl_rng **r, sIndiv **pTarget); //16
double probMoveP3(gsl_rng **r, sIndiv **pTarget); //17
double probOutsideVisit(gsl_rng **r, sIndiv **pTarget, int currDay);
...
double probInfectIndiv(sIndiv **pTarget, int currDay); //18
...
int main(int argc, const char * argv[])
{
    ...
    int vaxCapInmates; //5%
    int vaxCapPS;
    int vaxCapHS; //hsDose1Cap
    int inmateVaxD1=0; //cumulative count
    int inmateVaxD2=0; // cumulative count
    int hsVaxD1=0, hsVaxD2=0, psVaxD1=0, psVaxD2=0; // cumulative count
    int InmateV1=0, InmateV2=0, PSV1=0, PSV2=0, HSV1=0, HSV2=0; // daily count currently in prison
    ...
    fprintf(fw, "cumIVax1\t");
    fprintf(fw, "cumIVax2\t");
    fprintf(fw, "cumPSVax1\t");
    fprintf(fw, "cumPSVax2\t");
    fprintf(fw, "cumHSVax1\t");
    fprintf(fw, "cumHSVax2\t");
    fprintf(fw, "vaxCapInmate\t");
    fprintf(fw, "vaxCapPS\t");
    fprintf(fw, "vaxCapHS");
    fprintf(fw, "\n");
...
//    totalTestPS=0, totalTestHS=0, totalTPPS=0, totalTPHS=0, totalTNPS=0, totalTNHS=0, totalFPPS=0, totalFPHS=0, totalFNPS=0, totalFNHS=0;
    ...
    while(target!=NULL){
        if(target->indivType==1||target->indivType==2){
            target->vaccineProtection=0.79;
            target->vaccineTransmissionReduction=0.65;
            target->vaxDose1=1;
            target->vaxDose2=1;
            target->vaxDose1Day=currDay;
            target->vaxDose2Day=currDay;
        }
    ...
    */
    ...
    while(currDay!=daysSim+1){
        ...
        */
        ...
        //vaxx how many capacity
        //    int vaxCapInmates=1000; //5%
        //int vaxCapPS=1000;
        //int vaxCapHS=100; //hsDose1Cap
        //int inmatesVaxD1=0; //cumulative count
        //int inmatesVaxD2=0; // cumulative count
        //int hsVaxD1=0, hsVaxD2, psVaxD1, psVaxD2; // cumulative count
        //int InmateV1=0, InmateV2=0, PSV1=0, PSV2=0, HSV1=0, HSV2=0; // daily count currently in prison
        vaxCapInmates=0; //80%
        vaxCapPS=0;
        vaxCapHS=0; //hsDose1Cap
        
        vaxInmates(&nHead, &nTail, currDay, &vaxCapInmates, &inmateVaxD1, &inmateVaxD2);
        vaxPStaff(&nHead, &nTail, currDay, &vaxCapPS, &psVaxD1, &psVaxD2);
        vaxHStaff(&nHead, &nTail, currDay, &vaxCapHS, &hsVaxD1, &hsVaxD2);
...
*/
        ...
        while(target!=NULL){
            ...
            probEvents[0]=probMoveCom(&r, &target, currDay);//*rate of release to community
            probEvents[1]=probMoveP1(&r, &target, currDay);//*rate of movement to Prison 1
            ...
            probEvents[4]=probOutsideVisit(&r, &target, currDay); //and probability of getting infected
            ...
            probEvents[12]=probInfectIndiv(&target, currDay);
							...
							printf("PERFORM!\n");
            				 switch(iCrawl){
                                 case 0://move to community
                                     ...
                                     moveLocation(&r, &target, pLocArray, 0, pMinCellArray, pMedCellArray, pMaxCellArray, pIsolateMinZone, pIsolateMedZone, pIsolateMaxZone, pIsolateMinArea, pIsolateMedArea, pIsolateMaxArea, pIsolateMinUnit, pIsolateMedUnit, pIsolateMaxUnit, pIsolateMinCell, pIsolateMedCell, pIsolateMaxCell); //Community
                                 ...
                                 case 1://move to prison 1
                                     ...
                                     moveLocation(&r, &target, pLocArray, 1, pMinCellArray, pMedCellArray, pMaxCellArray, pIsolateMinZone, pIsolateMedZone, pIsolateMaxZone, pIsolateMinArea, pIsolateMedArea, pIsolateMaxArea, pIsolateMinUnit, pIsolateMedUnit, pIsolateMaxUnit, pIsolateMinCell, pIsolateMedCell, pIsolateMaxCell); //Prison 1
                                 ...
                                 case 2://move to prison 2
                                     ...
                                     moveLocation(&r, &target, pLocArray, 2, pMinCellArray, pMedCellArray, pMaxCellArray, pIsolateMinZone, pIsolateMedZone, pIsolateMaxZone, pIsolateMinArea, pIsolateMedArea, pIsolateMaxArea, pIsolateMinUnit, pIsolateMedUnit, pIsolateMaxUnit, pIsolateMinCell, pIsolateMedCell, pIsolateMaxCell); //Prison 2
                                     //nMovingPop[target->truckNumber]++;
                                     if (target->COVID==1){
                                         success=infectInTransit(&r, &target, &nHead, &nTail, pLocArray, &truckCIArray, &newInfectedInmates, &newInfectedPS, &newInfectedHS, &newInfectedEV, &newInfectedFV, &newInfectedInmatesMin, &newInfectedPSMin, &newInfectedHSMin, &newInfectedEVMin, &newInfectedFVMin, &newInfectedInmatesMed, &newInfectedPSMed, &newInfectedHSMed, &newInfectedEVMed, &newInfectedFVMed, &newInfectedInmatesMax, &newInfectedPSMax, &newInfectedHSMax, &newInfectedEVMax, &newInfectedFVMax, &totalAge0, &totalAge1, &totalAge2, &totalAge3, &totalAge4, &totalAge5, &totalAge6, currDay);
                                         if(success>0){ // success indicates number of new cases from this individual
                                 ...
                                 case 3://move to prison 3
                                     ...
                                     moveLocation(&r, &target, pLocArray, 3, pMinCellArray, pMedCellArray, pMaxCellArray, pIsolateMinZone, pIsolateMedZone, pIsolateMaxZone, pIsolateMinArea, pIsolateMedArea, pIsolateMaxArea, pIsolateMinUnit, pIsolateMedUnit, pIsolateMaxUnit, pIsolateMinCell, pIsolateMedCell, pIsolateMaxCell); //Prison 3
                                     //nMovingPop[target->truckNumber]++;
                                     if (target->COVID==1){
                                         success=infectInTransit(&r, &target, &nHead, &nTail, pLocArray, &truckCIArray, &newInfectedInmates, &newInfectedPS, &newInfectedHS, &newInfectedEV, &newInfectedFV, &newInfectedInmatesMin, &newInfectedPSMin, &newInfectedHSMin, &newInfectedEVMin, &newInfectedFVMin, &newInfectedInmatesMed, &newInfectedPSMed, &newInfectedHSMed, &newInfectedEVMed, &newInfectedFVMed, &newInfectedInmatesMax, &newInfectedPSMax, &newInfectedHSMax, &newInfectedEVMax, &newInfectedFVMax, &totalAge0, &totalAge1, &totalAge2, &totalAge3, &totalAge4, &totalAge5, &totalAge6, currDay);
                                         if(success>0){ // success indicates number of new cases from this individual
        ...
        if(nTail!=NULL){
            countVax(&nHead, &nTail, &InmateV1, &InmateV2, &PSV1, &PSV2, &HSV1, &HSV2);
            //InmateV1=0, InmateV2=0, PSV1=0, PSV2=0, HSV1=0, HSV2=0;
        }
        //totalTestPS=0, totalTestHS=0, totalTPPS=0, totalTPHS=0, totalTNPS=0, totalTNHS=0, totalFPPS=0, totalFPHS=0, totalFNPS=0, totalFNHS=0;
        ...
        fprintf(fw, "%d\t", InmateV1);
        fprintf(fw, "%d\t", InmateV2);
        fprintf(fw, "%d\t", PSV1);
        fprintf(fw, "%d\t", PSV2);
        fprintf(fw, "%d\t", HSV1);
        fprintf(fw, "%d\t", HSV2);
        fprintf(fw, "%d\t", inmateVaxD1);
        fprintf(fw, "%d\t", inmateVaxD2);
        fprintf(fw, "%d\t", psVaxD1);
        fprintf(fw, "%d\t", psVaxD2);
        fprintf(fw, "%d\t", hsVaxD1);
        fprintf(fw, "%d\t", hsVaxD2);
        fprintf(fw, "%d\t", vaxCapInmates);
        fprintf(fw, "%d\t", vaxCapPS);
        fprintf(fw, "%d\t", vaxCapHS);
        fprintf(fw, "\n");
        ...
        //vaxCapInmates=1000; //5%
        //vaxCapPS=1000;
        //vaxCapHS=100; //hsDose1Cap
