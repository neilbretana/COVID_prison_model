                                ...
                                if(tempCurrent->locArea==target->locArea&&tempCurrent->locUnit==target->locUnit&&tempCurrent->locCell==target->locCell){
                                    ...
                                    if(tempCurrent->COVID==0&&tempCurrent->COVIDAb==1){ //immunity
                                        ...
                                        if(target->severity==4||target->severity==5){ //mod or severe
                                            ...
                                            probsMask[0]=1-0.5;//1-0.75;//1-0.80; //probability of not being viraemic (Has antibodies)
                                            probsMask[1]=0.5;//0.75;//0.80; //probability of being viraemic (no antibodies)
                                            binMask=draw_multinom(&rCopy, 2, probsMask);
                                            if(binMask!=1){
                                                probInf[1]=probInf[1]-(probInf[1]*0.67); //assuming face mask 50% of the time
                                            }
