                    ...
                    if((current->PCRdate)<currDay){//if PCR tested since yesterday
                        if(current->COVID==1){
                            ...
                            if(lockLocation==1){ //min
                                (*pLockdownMinArea)[lockPrison][lockArea]=1;
                            ...
                            while(lockCurrent!=NULL){
                                if(lockCurrent->location==lockLocation){ //if same location
                                    if(lockCurrent->zone==lockPrison&&lockCurrent->locArea==lockArea){ //if same prison
...
void unlock(gsl_rng **r, sIndiv **pHeadCopy, sIndiv **pTailCopy, int currDay, int (*pLockdownMinZone)[27], int (*pLockdownMedZone)[11], int (*pLockdownMaxZone)[18], int (*pLockdownMinArea)[27][2], int (*pLockdownMedArea)[11][2], int (*pLockdownMaxArea)[18][4], int (*pLockdownMinUnit)[27][2][6], int (*pLockdownMedUnit)[11][2][4], int (*pLockdownMaxUnit)[18][4][5], int (*pIsolateMinZone)[27], int (*pIsolateMedZone)[11], int (*pIsolateMaxZone)[18], int (*pIsolateMinArea)[27][2], int (*pIsolateMedArea)[11][2], int (*pIsolateMaxArea)[18][4], int (*pIsolateMinUnit)[27][2][6], int (*pIsolateMedUnit)[11][2][4], int (*pIsolateMaxUnit)[18][4][5], int (*pIsolateMinCell)[27][2][6][13], int (*pIsolateMedCell)[11][2][4][19], int (*pIsolateMaxCell)[18][4][5][20]){
    ...
    for(ctr=0; ctr<lMinZone; ctr++){
                ...
                if(current->zone==ctr){ //This should be checking againts ctr. Check prison lockdown code
                            if(current->lockdown==1){
                                if(current->COVID==1){
                                    ...
                                    tempMinArea[ctr][targetArea]=1;//fill flag array (*flagLockdownMaxArea)[ctr][actr]= temp variable
                                }
                            }
                }
            }
        ...
        for(actr=0; actr<lMinArea; actr++){
            prevState=(*pLockdownMinArea)[ctr][actr];
            
            if (tempMinArea[ctr][actr]==0&&prevState==1) { // then Unlock
                current=*pHeadCopy;
                //if(infFlag==0){
                    //iterate through all
                    while(current!=NULL){
                        if(current->location==1){
                            if(current->zone==ctr&&current->locArea==actr){
                                if(current->lockdown==1){
    ...
    for(ctr=0; ctr<lMedZone; ctr++){
            ...
            if(current->location==2){
                if(current->zone==ctr){ //This should be checking againts ctr. Check prison lockdown code
                            if(current->lockdown==1){
                                if(current->COVID==1){
                                    targetArea=current->locArea;
                                    tempMedArea[ctr][targetArea]=1;//fill flag array (*flagLockdownMaxArea)[ctr][actr]= temp variable
                                }
                            }
                }
            }
        ...
        for(actr=0; actr<lMedArea; actr++){
            prevState=(*pLockdownMedArea)[ctr][actr];
            
            if (tempMedArea[ctr][actr]==0&&prevState==1) { // then Unlock
                current=*pHeadCopy;
                //if(infFlag==0){
                    //iterate through all
                    while(current!=NULL){
    ...
    for(ctr=0; ctr<lMaxZone; ctr++){
            ...
            if(current->location==3){
                if(current->zone==ctr){ //This should be checking againts ctr. Check prison lockdown code
                            if(current->lockdown==1){
                                if(current->COVID==1){
                                    targetArea=current->locArea;
                                    tempMaxArea[ctr][targetArea]=1;//fill flag array (*flagLockdownMaxArea)[ctr][actr]= temp variable
                                }
                            }
                }
            }
        ...
        for(actr=0; actr<lMaxArea; actr++){
            prevState=(*pLockdownMaxArea)[ctr][actr];
            
            if (tempMaxArea[ctr][actr]==0&&prevState==1) { // then Unlock
                current=*pHeadCopy;
                //if(infFlag==0){
                    //iterate through all
                    while(current!=NULL){
