            ...
            if(current->severity>=3){ //have symptoms
                if(current->PCRtested==1){
                    if((current->PCRdate)<currDay){//if PCR tested since yesterday
                        if(current->COVID==1){
                            //set area/unit/prison in lockdown via pLockdownMinArea
                            lockLocation=current->location;
                            lockPrison=current->zone;
                            lockArea=current->locArea;
                            lockUnit=current->locUnit;
                            lockCell=current->locCell;
                            current->lockdown=1;
                            current->timeLockdown=currDay;
                            //current->isolated=2; // 1 for 1 out isolation, 2 for 2 out
                            //current->timeIsolated=currDay;
                            
                            if(lockLocation==1){ //min
                                (*pIsolateMinCell)[lockPrison][lockArea][lockUnit][lockCell]=1;
                                //(*pLockdownMinZone)[lockPrison]=1;
                                //(*pIsolateMinCell)[current->zone][current->locArea][current->locUnit][current->locCell]=2;
                            }else if(lockLocation==2){ //med
                                (*pIsolateMedCell)[lockPrison][lockArea][lockUnit][lockCell]=1;
                                //(*pIsolateMedCell)[current->zone][current->locArea][current->locUnit][current->locCell]=2;
                            }else if(lockLocation==3){ //max
                                (*pIsolateMaxCell)[lockPrison][lockArea][lockUnit][lockCell]=1;
                                //(*pIsolateMaxCell)[current->zone][current->locArea][current->locUnit][current->locCell]=2;
                            }
                            
                            //lockdown inmates who are in this prison straight away
                            //quarantine inmates
                            while(lockCurrent!=NULL){
                                if(lockCurrent->location==lockLocation){ //if same location
                                    if(lockCurrent->zone==lockPrison&&lockCurrent->locArea==lockArea&&lockCurrent->locUnit==lockUnit&&lockCurrent->locCell==lockCell){ //if same prison
                                        if(lockCurrent->lockdown!=1){ //if not yet in lockdown
                                            //put in lockdown and 2-out isolation
                                            lockCurrent->lockdown=1;
                                            lockCurrent->timeLockdown=currDay;
                                            //lockCurrent->isolated=2; // 1 for 1 out isolation, 2 for 2 out
                                            //lockCurrent->timeIsolated=currDay;
                                ...
                                lockCurrent=lockCurrent->nextIndiv;
                            }
                ...
                }else if(current->PCRtested==0&&current->lockdown==0){//if not PCR tested
                    current->PCRtested=1;
...
void unlock(gsl_rng **r, sIndiv **pHeadCopy, sIndiv **pTailCopy, int currDay, int (*pLockdownMinZone)[27], int (*pLockdownMedZone)[11], int (*pLockdownMaxZone)[18], int (*pLockdownMinArea)[27][2], int (*pLockdownMedArea)[11][2], int (*pLockdownMaxArea)[18][4], int (*pLockdownMinUnit)[27][2][6], int (*pLockdownMedUnit)[11][2][4], int (*pLockdownMaxUnit)[18][4][5], int (*pIsolateMinZone)[27], int (*pIsolateMedZone)[11], int (*pIsolateMaxZone)[18], int (*pIsolateMinArea)[27][2], int (*pIsolateMedArea)[11][2], int (*pIsolateMaxArea)[18][4], int (*pIsolateMinUnit)[27][2][6], int (*pIsolateMedUnit)[11][2][4], int (*pIsolateMaxUnit)[18][4][5], int (*pIsolateMinCell)[27][2][6][13], int (*pIsolateMedCell)[11][2][4][19], int (*pIsolateMaxCell)[18][4][5][20]){
    ...
    int tempMinZone[27]; //to check if a prison is in lockdown
    int tempMinArea[27][2];
    int tempMinUnit[27][2][6];
    int tempMinCell[27][2][6][13];
    
    int mcCtr1, mcCtr2, mcCtr3, mcCtr4;
    for(mcCtr1=0; mcCtr1<27; mcCtr1++){ //correctional centres
        tempMinZone[mcCtr1]=0;
        for(mcCtr2=0; mcCtr2<2; mcCtr2++){ //min med max
            tempMinArea[mcCtr1][mcCtr2]=0;
            for(mcCtr3=0; mcCtr3<6; mcCtr3++){ //min med max
                tempMinUnit[mcCtr1][mcCtr2][mcCtr3]=0;
                for(mcCtr4=0; mcCtr4<13; mcCtr4++){ //min med max
                    tempMinCell[mcCtr1][mcCtr2][mcCtr3][mcCtr4]=0;
                }
            }
        }
    }
    
    int tempMedZone[11]; //[correctional centres][indivTypes]
    int tempMedArea[11][2];
    int tempMedUnit[11][2][4];
    int tempMedCell[11][2][4][19];
    
    for(mcCtr1=0; mcCtr1<11; mcCtr1++){ //correctional centres
        tempMedZone[mcCtr1]=0;
        for(mcCtr2=0; mcCtr2<2; mcCtr2++){ //min med max
            tempMedArea[mcCtr1][mcCtr2]=0;
            for(mcCtr3=0; mcCtr3<4; mcCtr3++){ //min med max
                tempMedUnit[mcCtr1][mcCtr2][mcCtr3]=0;
                for(mcCtr4=0; mcCtr4<19; mcCtr4++){ //min med max
                    tempMedCell[mcCtr1][mcCtr2][mcCtr3][mcCtr4]=0;
                }
            }
        }
    }
    
    int tempMaxZone[18]; //[correctional centres][indivTypes]
    int tempMaxArea[18][4];
    int tempMaxUnit[18][4][5];
    int tempMaxCell[18][4][5][20];
    
    for(mcCtr1=0; mcCtr1<18; mcCtr1++){ //correctional centres
        tempMaxZone[mcCtr1]=0;
        for(mcCtr2=0; mcCtr2<4; mcCtr2++){ //min med max
            tempMaxArea[mcCtr1][mcCtr2]=0;
            for(mcCtr3=0; mcCtr3<5; mcCtr3++){ //min med max
                tempMaxUnit[mcCtr1][mcCtr2][mcCtr3]=0;
                for(mcCtr4=0; mcCtr4<20; mcCtr4++){ //min med max
                    tempMaxCell[mcCtr1][mcCtr2][mcCtr3][mcCtr4]=0;
                }
            }
        }
    }
    ...
    //create flag array (*flagLockdownMaxArea)[ctr][actr]= with all 0s;
    
    //check Min prisons first
    for(ctr=0; ctr<lMinZone; ctr++){
        current=*pHeadCopy;
        infFlag=0;
        //iterate through inmates in the same zone
        printf("going through Min prisons\n");
        while(current!=NULL){
            if(current->location==1){
                if(current->zone==ctr){ //This should be checking againts ctr. Check prison lockdown code
                            if(current->lockdown==1){
                                if(current->COVID==1){
                                    targetArea=current->locArea;
                                    targetUnit=current->locUnit;
                                    targetCell=current->locCell;
                                    tempMinCell[ctr][targetArea][targetUnit][targetCell]=1;//fill flag array (*flagLockdownMaxArea)[ctr][actr]= temp variable
                                }
                            }
                }
            }
        ...
        printf("checking for min unit to unlock\n");
        //run through flag array, if (*pIsolateMinCell)[current->zone][current->locArea]==1; AND (*flagIsolateMinCell)[current->zone][current->locArea]==0
                   //THEN run through all inmates and unlock all inmates in that zone and area
        
        for(actr=0; actr<lMinArea; actr++){
            printf("going through min areas\n");
            for(uctr=0; uctr<lMinUnit; uctr++){
                printf("going through min units\n");
                    for(cctr=0; cctr<lMinCell; cctr++){
                        prevState=(*pIsolateMinCell)[ctr][actr][uctr][cctr];
                            
                        if (tempMinCell[ctr][targetArea][targetUnit][targetCell]==0&&prevState==1) { // then Unlock
                            current=*pHeadCopy;
                            //if(infFlag==0){
                                //iterate through all
                                while(current!=NULL){
                                    if(current->location==1){
                                        if(current->zone==ctr&&current->locArea==actr&&current->locUnit==uctr&&current->locCell==cctr){
                                            if(current->lockdown==1){
                                                //set lockdown to 0, isolated to 0
                                                current->lockdown=0;
                                                current->timeLockdown=0;
                                                //current->isolated=0; // 1 for 1 out isolation, 2 for 2 out
                                                //current->timeIsolated=0;
                                                //(*pIsolateMinCell)[current->zone][current->locArea][current->locUnit][current->locCell]=0;
                                            }
                                        }
                                ...
                                //set lockdown to 0
                                (*pIsolateMinCell)[ctr][actr][uctr][cctr]=0;
                            //}
                        }
        ...
        printf("done unlocking min\n");
    }
    
    //check Med prisons
    for(ctr=0; ctr<lMedZone; ctr++){
        current=*pHeadCopy;
        infFlag=0;
        //iterate through inmates in the same zone
        printf("going through Med prisons\n");
        while(current!=NULL){
                ...
                if(current->zone==ctr){ //This should be checking againts ctr. Check prison lockdown code
                            if(current->lockdown==1){
                                if(current->COVID==1){
                                    ...
                                    targetUnit=current->locUnit;
                                    targetCell=current->locCell;
                                    tempMedCell[ctr][targetArea][targetUnit][targetCell]=1;//fill flag array (*flagLockdownMaxArea)[ctr][actr]= temp variable
                                }
                            }
                }
            }
        ...
        printf("checking for med unit to unlock\n");
        //run through flag array, if (*pIsolateMinCell)[current->zone][current->locArea]==1; AND (*flagIsolateMinCell)[current->zone][current->locArea]==0
                   //THEN run through all inmates and unlock all inmates in that zone and area
        
        for(actr=0; actr<lMedArea; actr++){
            printf("going through med areas\n");
            for(uctr=0; uctr<lMedUnit; uctr++){
                printf("going through med units\n");
                    for(cctr=0; cctr<lMedCell; cctr++){
                        prevState=(*pIsolateMedCell)[ctr][actr][uctr][cctr];
                            
                        if (tempMedCell[ctr][targetArea][targetUnit][targetCell]==0&&prevState==1) { // then Unlock
                            current=*pHeadCopy;
                            //if(infFlag==0){
                                //iterate through all
                                while(current!=NULL){
                                ...
                                //set lockdown to 0
                                (*pIsolateMedCell)[ctr][actr][uctr][cctr]=0;
                            //}
                        }
        ...
        printf("done unlocking med\n");
    }
    
    //check Max prisons
    for(ctr=0; ctr<lMaxZone; ctr++){
        current=*pHeadCopy;
        infFlag=0;
        //iterate through inmates in the same zone
        printf("going through Max prisons\n");
        while(current!=NULL){
                ...
                if(current->zone==ctr){ //This should be checking againts ctr. Check prison lockdown code
                            if(current->lockdown==1){
                                if(current->COVID==1){
                                    ...
                                    targetUnit=current->locUnit;
                                    targetCell=current->locCell;
                                    tempMaxCell[ctr][targetArea][targetUnit][targetCell]=1;//fill flag array (*flagLockdownMaxArea)[ctr][actr]= temp variable
                                }
                            }
                }
            }
        ...
        printf("checking for max unit to unlock\n");
        //run through flag array, if (*pIsolateMinCell)[current->zone][current->locArea]==1; AND (*flagIsolateMinCell)[current->zone][current->locArea]==0
                   //THEN run through all inmates and unlock all inmates in that zone and area
        
        for(actr=0; actr<lMaxArea; actr++){
            printf("going through max areas\n");
            for(uctr=0; uctr<lMaxUnit; uctr++){
                printf("going through max units\n");
                    for(cctr=0; cctr<lMaxCell; cctr++){
                        prevState=(*pIsolateMaxCell)[ctr][actr][uctr][cctr];
                            
                        if (tempMaxCell[ctr][targetArea][targetUnit][targetCell]==0&&prevState==1) { // then Unlock
                            current=*pHeadCopy;
                            //if(infFlag==0){
                                //iterate through all
                                while(current!=NULL){
                                ...
                                //set lockdown to 0
                                (*pIsolateMaxCell)[ctr][actr][uctr][cctr]=0;
                            //}
                        }
...
double probMoveP1(gsl_rng **r, sIndiv **pTarget, int currDay){ //Probability to move to MIN
    sIndiv *current;
    current=*pTarget;
    double rN, lb, ub;
    rN=0.0;
...
double probOutsideVisit(gsl_rng **r, sIndiv **pTarget, int currDay){ //Probability to move to MAX
    sIndiv *current;
    current=*pTarget;
    double rN=0.0, lb, ub;
