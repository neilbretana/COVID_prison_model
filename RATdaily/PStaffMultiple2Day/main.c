...
int main(int argc, const char * argv[])
{
    ...
    fprintf(fw, "TotalRTestPS\tTotalRTestHS\tTotalRTTPPS\tTotalRTTPHS\tTotalRTTNPS\tTotalRTTNHS\tTotalRTFPPS\tTotalRTFPHS\tTotalRTFNPS\tTotalRTFNHS\tOvercap\n");
...
//    totalTestPS=0, totalTestHS=0, totalTPPS=0, totalTPHS=0, totalTNPS=0, totalTNHS=0, totalFPPS=0, totalFPHS=0, totalFNPS=0, totalFNHS=0;
    ...
    for(k=0;k<18;k++){
        if(k==11){
            ...
            newIndiv(&idG, &HCVe, &HCVeAb, currDay, &nHead, &nTail, &r, ROWPRIS, pLocArray, 12, 99, 0, k, pMinCellArray, pMedCellArray, pMaxCellArray, pIsolateMinZone, pIsolateMedZone, pIsolateMaxZone, pIsolateMinArea, pIsolateMedArea, pIsolateMaxArea, pIsolateMinUnit, pIsolateMedUnit, pIsolateMaxUnit, pIsolateMinCell, pIsolateMedCell, pIsolateMaxCell, &overcap); //seed
        }else{
            for(pK=0; pK<120; pK++){
                newIndiv(&idG, &HCVe, &HCVeAb, currDay, &nHead, &nTail, &r, ROWPRIS, pLocArray, 12, 99, 1, k, pMinCellArray, pMedCellArray, pMaxCellArray, pIsolateMinZone, pIsolateMedZone, pIsolateMaxZone, pIsolateMinArea, pIsolateMedArea, pIsolateMaxArea, pIsolateMinUnit, pIsolateMedUnit, pIsolateMaxUnit, pIsolateMinCell, pIsolateMedCell, pIsolateMaxCell, &overcap);//new prisoner to be moved from com to a random prison based on **
            }
    ...
    for(k=0;k<18;k++){ //about 7 per Max prison
        if(k==11){
            ...
            //newIndiv(&idG, &HCVe, &HCVeAb, currDay, &nHead, &nTail, &r, ROWPRIS, pLocArray, 13, 99, 0, k, pMinCellArray, pMedCellArray, pMaxCellArray, pIsolateMinZone, pIsolateMedZone, pIsolateMaxZone, pIsolateMinArea, pIsolateMedArea, pIsolateMaxArea, pIsolateMinUnit, pIsolateMedUnit, pIsolateMaxUnit, pIsolateMinCell, pIsolateMedCell, pIsolateMaxCell, &overcap); //seed
        }else{
            for(sK=0; sK<7; sK++){ //7
                newIndiv(&idG, &HCVe, &HCVeAb, currDay, &nHead, &nTail, &r, ROWPRIS, pLocArray, 13, 99, 1, k, pMinCellArray, pMedCellArray, pMaxCellArray, pIsolateMinZone, pIsolateMedZone, pIsolateMaxZone, pIsolateMinArea, pIsolateMedArea, pIsolateMaxArea, pIsolateMinUnit, pIsolateMedUnit, pIsolateMaxUnit, pIsolateMinCell, pIsolateMedCell, pIsolateMaxCell, &overcap);//new prisoner to be moved from com to a random prison based on **
            }
    ...
    while(currDay!=daysSim+1){
        ...
        if(currDay<8){ //put in new infections in the first 7 days
            for(k=0;k<inPop;k++){
        ...
        //Infection in red zones
        //checkQuarantine
        //checkIsolate
        
        //rapidTest staff every morning before we begin
        entryFlag=0;
        if(currDay>1&&currDay<120){ //covid p staff every day
            ...
            while(entryFlag==0){
                if(target->indivType==1){
                    if(target->COVID==0){
                        target->COVID=1;
                        target->COVIDAb=0;
                        target->timeOfInfection=currDay;
                        target->severity=1;
                        entryFlag=1;
                    }
                }
...
*/
        ...
        newCases=0; infectedOutside=0; hCovid=0;
